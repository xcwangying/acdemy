...
class Fandan{
	...
	//返单分钱上级身份
	public $noble_status_msbfq = array(8,9,10);
	...
	/*public function digui1($ikey){ 
		...
		if(!$re){//没有第一级时结束
			...
			{//找卖手及以上分钱
		  		if( in_array($re['identity'], $this->noble_status_msfq) ) { 
		  			$this->one = $re;
		  		} else { 
		  			//递归找分钱上级 有卖手
		  			$this->digui1($re['ikey']);
	...
	public function handle($orid,$type){ 
		...
		if($this->orid){ 
			...
			if( $orderinfo ) { //存在订单,修改订单状态变为已支付
        		...
        		if( $oldvip['identity'] < 8 ) { 
        			M()->rollback();return false;exit();
        		}
        		$this->vipre = $oldvip;
        		$this->one = $oldvip;
				...
				$vfsre = $this->vip->where($vmap)->setInc("use_money",$orderinfo['order_amount']);//修改会员累计消费
				if( !$vfsre ) { M()->rollback();return false;exit(); }
				$vfs = $this->vip->where($vmap)->setInc("fandancs");//修改会员返单次数
        		//判断会员及订单修改是否成功
        		if($vfs && $ordes){
	...
	public function  return_goods($orid){
		...
		if($this->orid){
			...
			if($orderinfo){//存在订单,修改订单状态变为已退货，
				...
				$pidre = $this->goods->where($gmap)->getField("pid");//查询商品对应的项目分类 退货好像没用
				if( !$pidre ) return false; else $this->pid = $pidre;
	        	...
	        	$vfsre = $this->vip->where($vmap)->setDec("use_money",$orderinfo['order_amount']);//修改会员累计消费 //如果存在返单 是否还需要减少自己的小积分和累计积分
	        	if( !$vfsre ) { M()->rollback();return false;exit(); }
	        	$vfs = $this->vip->where($vmap)->setDec("fandancs");//修改会员返单次数
    ...
    public function jifenmb($vid,$money,$type){ 
    	...
    	if( $type == 1 ) { 
    		if( $this->one['vid'] == $vid ) 
    			$str1 = "您的返单积分奖励已到账，请在会员中心-佣金流水-中查看！";
    		else 
    			$str1 = "您的团队有成员下单，积分奖励已到账，请在会员中心-佣金流水-中查看！";
    	} else { 
    		if( $this->one['vid'] == $vid ) 
    			$str1 = "您的返单积分奖励已扣除，请在会员中心-佣金流水-中查看！";
    		else 
	...
	public function threeIdentity($one,$two,$three,$orderinfo,$type){
		if($type == '1'){
			...
			if( ($this->identityinfo['in_one'] > 0) && $one['vid']){//购买对应身份一级有分成
				...
				$icdata_one['chage_state'] = '11';//1:佣金 2:提现 
			...
			if(!$chiefdata){//没有上级
				...
				if(($chiefdata['omoney'] > 0) && $chiefdata['oid']){//购买对应身份一级有分成
					...
					$icdata_one['chage_state'] = '12';//1:佣金 2:提现  4-二级退货退积分 7退货退奖励金

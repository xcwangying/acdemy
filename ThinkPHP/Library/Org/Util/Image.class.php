...
class Image {
     ...
     * @return void
     */
    static function thumb($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true, $flag=0) {
        // 获取原图信息
        ...
        if ($info !== false) {
			...
			switch($flag){
				...
				$m=new UploadFileA();
					if(!upload_file_to_alioss($thumbname,$m->autoCharset(substr($thumbname,2),'utf-8','gbk'))) {
						$m->error = '文件上传保存错误！';
						return false;
					}
					if($flag==1){

            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    <div class="box box-primary" id="window_main">
                        <!-- /.box-header -->
                        <!-- form start -->
                        <form role="form" method="post" action="{:U('Article/doeditinstrument')}" name="frm" enctype="multipart/form-data" class="form-horizontal form-seperated shequn validform  window_main" cleandata="#pro_tbody">
                            <div class="box-body">
                                <div >
                                    <label>仪器名称：<span style="color: #FF0000;">*</span></label>
                                    <input type="text" class="form-control" datatype="*"  placeholder="(必填)请填写仪器名称！" name="goods_name" value="{$goods_info.goods_name}">
                                    <input type="hidden" name="goods_id" value="{$goods_info.goods_id}">
                                </div>

                                <div >
                                    <label>仪器价格：<span style="color: #FF0000;">*</span></label>
                                    <input type="text"class="form-control goods_price" datatype="currency" nullmsg="请填写仪器价钱" placeholder="(必填)请填写仪器价格！" errormsg="请正确填写金额且需小于100万！" name="goods_price" value="{$goods_info.goods_price}">
                                </div> 

                                <!-- <div>
                                    <label>项目分类：<span style="color: #FF0000;">*</span></label>
                                ...
                                <div>
                                    ...
                                    <select name="mid" class="form-control input-sm" style="width: auto;" id="Identity">
                                        ...
                                        <volist name="identityList" id="item">
                                            <option value="{$item.id}" <if condition="$item['id'] eq $goods_info['goods_identity']">selected</if>>{$item.identity}</option>
                                        </volist>
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" class="datetimepicker3 input_date type input-sm input_ser" readonly name="statime" value="{$goods_info['statime'] | date='Y-m-d ',###}"  style="height:30px;border:1px solid #d2d6de;border-radius: 0px;font-size:14px;" datatype="*"/>
                                </div>

                                <div>
                                    <label>报名截止时间：<span style="color: #FF0000;">*</span></label>
                                    <input type="text" class="datetimepicker3 input_date type input-sm input_ser" readonly name="endtime" value="{$goods_info['endtime'] | date='Y-m-d ',###}"  style="height:30px;border:1px solid #d2d6de;border-radius: 0px;font-size:14px;" datatype="*"/>
                                </div> 
                                ...
                                <div class="gs_display">
                                    ...
                                    <input type="text" class="form-control gs_display" datatype="currency" nullmsg="请填金额" placeholder="(必填)请填写金额！" errormsg="请正确填写金额且需小于10万！" name="gs_one" value="{$good_sjfx.gs_one}">
                                </div> 

                                <div class="gs_display">
                                    <label>第二级分成（金额）：<span style="color: #FF0000;">*</span></label>
                                    <input type="text" class="form-control gs_display" datatype="currency" nullmsg="请填金额" placeholder="(必填)请填写金额！" errormsg="请正确填写金额且需小于10万！" name="gs_two" value="{$good_sjfx.gs_two}">
                                </div> 

                                <div class="gs_display">
                                    <label>第三级分成（金额）：<span style="color: #FF0000;">*</span></label>
                                    <input type="text" class="form-control gs_display" datatype="currency" nullmsg="请填金额" placeholder="(必填)请填写金额！" errormsg="请正确填写金额且需小于10万！" name="gs_three" value="{$good_sjfx.gs_three}">
                                </div> 

                                <div class="gs_display">
                                    <label>平台分成（百分比）：<span style="color: #FF0000;">*</span></label>
                                    <input type="text" class="form-control gs_display" datatype="ns" nullmsg="请填0到100以内的整数" placeholder="(必填)请填0到100以内的整数！" errormsg="请填0到100以内的整数" name="pingtai" value="{$good_sjfx.pingtai}">
                                </div>  -->


                                <label>仪器主图：<span style="color: #FF0000;">* （图片尺寸640*300）</span></label>
                                <!-- 待裁剪图片的div -->
                                ...
                                <div style="height:<?php echo $pic_info['h']*0.3?>px;">
                                    ...
                                    <div id="goods_image_view" class="view" <if condition="$goods_info.goods_image">style='background-image: url("<?php echo C('yuming'); ?>/{$goods_info.goods_image}");background-size:100% 100%;'<else/>style='margin-right:45px'</if>>
                                ...
                                <div >  
                                    ...
                                    <script id="container" name="goods_body" type="text/plain">{$goods_info.goods_body}</script>
                                </div>

                            <div class="box-footer">
                                <button type="submit" class="btn btn-primary">提交</button>
                                <a href="{:U('Article/instrument')}" class="btn btn-info">返回</a>
                            </div>
                            </div>
                        </form>
                    </div>
...
<!-- add by liu 图片裁剪 -->
    ...
    <script>
        ...
        var goods_prices = $('.goods_price').val();
        if( goods_prices*1 > 0 ) { 
            $('.gs_display').css('display','block');
        } else { 
            $('.gs_display').css('display','none');
        ...
        $.ajax({
            ...
            success:function(re){
                ...
                if(re.status == 1){
                    ...
                    window.setTimeout("window.location='"+"{:U('Article/instrument')}'",1500);
                }else{
                    art.dialog({
                      content:re.info,
                      lock:true
                    ...
                    sub.val('提交');
                    sub.attr('disabled',false);
                    //window.setTimeout(window.location="{:U('Article/instrument')}",2000);
                }
            }
        })      
        return false;

...
<script src="__Assert__/js/jquery-2.2.3.js"></script>
<link rel="stylesheet" href="__Assert__/css/jquery-ui.min.css">
<script src="__Assert__/js/jquery-ui-1.9.2.min.js"></script>
...
.btn, .btn_active {
    ...
    font-size: 14px;
    float: left;
    background: url(__Assert__/img/btn.png);
    margin-top: 30px;
    text-align: center;
    cursor: pointer;
}
...
.btn_active {
    background: url(__Assert__/img/btn.png) -49px 0px;
}
...
<div style="height:53px;">
    <div class="search">
        <div class="search_c"><input type="text" class="search_t" style='padding:5px;padding-bottom: 10px;' onKeyPress="if(event.keyCode==13) {btnSearch.click();return false;}"/></div>
        <div id="btn_search" class="btn" style='padding:1px 12px;margin-top:10px;'>搜索</div>
    </div>
<!--    <div class="poi">
...
qq.maps.event.addListener(map, "click", function (e) {
    ...
    // url3 = encodeURI("http://apis.map.qq.com/ws/geocoder/v1/?location=" + e.latLng.getLat() + "," + e.latLng.getLng() + "&key=F3NBZ-AD46X-KOA4L-7E3ND-UI27J-TLBXK&output=jsonp&&callback=?");
    ...
    //在原版上级进行修改
    top.setpoint(e.latLng.getLat().toFixed(6), e.latLng.getLng().toFixed(6));
    return;
...
qq.maps.event.addDomListener(btnSearch, 'click', function () {
    ...
    url = encodeURI("http://apis.map.qq.com/ws/place/v1/search?keyword=" + value + "&boundary=region(" + query_city + ",0)&page_size=9&page_index=1&key=F3NBZ-AD46X-KOA4L-7E3ND-UI27J-TLBXK&output=jsonp&&callback=?");
    $.getJSON(url, function (result) {
        ...
        if (result.count) {
            ...
            each(result.data, function (n, ele) {
                ...
                var leftn = -54 - 17 * n;
                order.style.cssText = "width:17px;height:17px;margin:3px 3px 0px 0px;float:left;background:url(__Assert__/img/marker_n.png) " + leftn + "px 0px";
...
function setAnchor(marker, flag) {
    ...
    if (flag == true) {
        var anchor = new qq.maps.Point(10, 30),
                ...
                icon = new qq.maps.MarkerImage("__Assert__/img/marker10.png", size, origin, anchor);
        marker.setIcon(icon);
    } else {
        var anchor = new qq.maps.Point(10, 30),
...
function setFlagClicked(arr, index) {
    each(markerArray, function (n, ele) {
        if (n == index) {
        	...
        	//在原版上修改
    		top.setpoint(latLng.getLat().toFixed(6), latLng.getLng().toFixed(6));
    		return;
        } else {
            ele.isClicked = false;
...
city.onclick = function (e) {
    ...
    if (target.className == "city_name") {
        ...
        url2 = encodeURI("http://apis.map.qq.com/ws/geocoder/v1/?region=" + curCity.children[0].innerHTML + "&address=" + curCity.children[0].innerHTML + "&key=F3NBZ-AD46X-KOA4L-7E3ND-UI27J-TLBXK&output=jsonp&&callback=?");
...
$(".search_t").autocomplete({
    source:function(request,response){
        url4 = encodeURI("http://apis.map.qq.com/ws/place/v1/suggestion/?keyword=" + request.term + "&region=" + curCity.children[0].innerHTML + "&key=F3NBZ-AD46X-KOA4L-7E3ND-UI27J-TLBXK&output=jsonp&&callback=?");
